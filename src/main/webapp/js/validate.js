...
function validate() {
    ...
    if(companyPages==''){
        humane.error("请输入企业主页地址!");
        return ;
    }else {
        url=new RegExp("^(http|https)://([\\w-]+\\.)+[\\w-]+(/[\\w-./?%&=]*)?$");
        if(checkValidateNum(companyPages,url)==false){
            humane.error("企业地址格式错误! 正确格式:http://www.baidu.com");
            return;
        }

    }
    if(companyStatus==''){
        humane.error("请选择启用状态!");
        return ;
    }
    if(legalRepresentative.length==0)
    {
        humane.error("法人代表不能为空!");
        return ;
    }
    if(cardnum.length==0)
    {
        humane.error("证件号码不能为空!");
        return ;
    }else {
        url=/^[0-9]*$/;
        if(checkValidateNum(cardnum,url)==false){
            humane.error("证件号码必须为数字!");
            return;
        }
    }

    if(customcardtype==''){
        humane.error("请选择证件类型!");
        return ;
    }
    if(customprovince==''){
        humane.error("请选择省份!");
    ...
    for (var i=0;i<fax.length;i++){
        ...
        if(checkValidateNum(fax[i].value,url)==false){
            humane.error("传真格式错误! 正确格式:0111-1111111");
            return;
        }
    }
    ...
    for (var i=0;i<email.length;i++){
        ...
        if(checkValidateNum(email[i].value,url)==false){
            humane.error("邮箱格式错误! 正确格式:zhangsan-001@gmail.com");
            return;
        }
    }
}